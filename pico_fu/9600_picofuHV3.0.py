...
pass
# send dummy command
PIco.write(":020000040000FAr")
except:
print "KOnERROR: Unable to establish communication with PIco bootloader via port:",self.port,'Please verify that the serial port is availble.'
sys.exit(2)
try:
...
print "KOnERROR: Invalid response from PIco:",ord(resp[0])," Please retry the FW upload process."
sys.exit(2)
break
rcnt-=1
if (rcnt==1000):
...
print "KOnERROR: Something wrong happened during verification of communication channel with PIco bootloader via port:",self.port,'Please verify that the serial port is availble and not used by some other application.'
sys.exit(2)

# in case communication not verified
if not status:
if self.force:
print "KOnWARNING: Unable to verify communication with bootloader in PIco. Is the PIco in the bootloader mode? (Red LED lid on PIco)"
else:
print "KOnERROR: Failed to establish communication with bootloader in PIco. Is the PIco in the bootloader mode? (Red LED lid on PIco)"
sys.exit(2)
...
print "KOnLine",lnum,': Invalid bytecode message!'
sys.exit(4)
...
# validate the CRC :)
if crc1!=crc2:
print "KOnLine",lnum,': Invalid bytecode checksum! Defined:', crc1,'Calculated:', crc2
sys.exit(4)
...
f.close()
if not valid:
print "KOn Termination signature not found in the firmware file."
sys.exit(4)
else:
print 'OK'
...
print "KOnERROR: Invalid status word from PIco (",ord(resp),') when processing initial line! Please retry the FW upload process.'
sys.exit(5)
break
rcnt-=1
if not status:
print "KOnERROR: No status word from PIco revcieved when processing initial line! Please check possible warnings above and retry the FW upload process."
sys.exit(5)
...
print "KOnERROR: Invalid status word from PIco (",ord(resp),') when processing line',lnum2,' Please retry the FW upload process.'
sys.exit(5)
break
rcnt-=1
if not status:
print "KOnERROR: No status word from PIco revcieved when processing line",lnum2,' Please check possible warnings above and retry the FW upload process.'
sys.exit(5)
# in case that the done command is detected, than finish
if line[7:9]=='01':
break
...
def usage(self):
print "n",sys.argv[0],' -f <fw_file> [ -v | -h | --force | -s | -p serial | -b i2c_bus_number ]',"n"
sys.exit(1)
